...
The pinout below is for the 
RFID breakout and Arduino Uno/
Sparkfun RedBoard:
...
Connect the servo to VCC, GND,
and Arduino D9

You can also put an LED & 330 ohm
resistor between the RFID module's
READ pin and GND for a "card 
successfully read" indication
...
void loop() {
  ...
  if (strlen(newTag)== 0) {
    ...
    // This loop checks newTag against
    // every tag in the knownTags array
    ...
    if (total > 0) {
      // Put the action of your choice in this loop!
